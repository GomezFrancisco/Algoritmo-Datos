...
class Grafo():
    ...
    def barrido_maravillas(self, dato):
        cont_arquitecturas = 0
        cont_naturales = 0
        for value in self.__elements:
            if dato in value[0].pais:
                if value[0].tipo == 'Arquitectonica':
                    cont_arquitecturas+=1
                else:
                    cont_naturales+=1
        if cont_arquitecturas != 0 or cont_naturales != 0:
            return f'{dato} tiene: Maravillas Arquitectonicas {cont_arquitecturas} - Maravillas Naturales {cont_naturales}'
        else:
            return f'{dato} no se encuentra en el grafo.'

    def barrido_tipos(self, dato):
        cont_arquitecturas = 0
        cont_naturales = 0
        for value in self.__elements:
            if dato in value[0].pais:
        ...
        if cont_arquitecturas >= 2 or cont_naturales >= 2:
            return f'{dato} tiene más de una maravillas de un mismo tipo.'
        elif cont_naturales == 1 or cont_arquitecturas == 1:
            return f'{dato}  no tiene más de una maravillas de un mismo tipo.'
        else:
            return f'{dato} no se encuentra en el grafo.'
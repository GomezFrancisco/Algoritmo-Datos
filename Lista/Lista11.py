...
J1 = personajes('a', 68, 1, 'femenino', 'droide', 'a',  [4, 5, 6])
J2 = personajes('b', 69, 2, 'femenino', 'droide', 'b', [5])
J3 = personajes('Han solo', 72, 849, 'masculino', 'droide', 'b', [6] )
...
for i in range(lista.size()):
    if (lista.get_element_by_index(i).episodio in episodio) and (lista.get_element_by_index(i).especie == 'droide'):
        print ('Droide: ', lista.get_element_by_index(i))
print()

#3
darth = lista.search('Darth Vader', 'nombre')
Han = lista.search('Han solo', 'nombre')
if darth != -1:
    print ('Darth vader se encuentra y sus datos son: ', lista.get_element_by_index(darth))
else:
    print ('Darth no se encuentra en la lista.')

print()

if Han != -1:
    print ('Han Solo se encuentra y sus datos son: ', lista.get_element_by_index(darth))
else:
    print ('Han Solo no se encuentra en la lista.')
...
if buscado != -1:
    print ('Chewbacca aparece en el capítulo: ', lista.get_element_by_index(buscado), ' y el resto de su información es: ', lista.get_element_by_index(posicion))
else:
